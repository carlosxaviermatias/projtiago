...
- [Sistema Enfermagem](project_sistema_enfermagem.md) — puericultura **EM PRODUÇÃO**, 108 crianças reais, import automático de CSV do e-SUS; **6 critérios × 1/6**, busca ativa do ACS, gráficos clicáveis, PDF, segurança auditada; ⚠️ LER SISTEMA-ENFERMAGEM.md antes de mexer
- [Site Josiane (enfermeira)](project_site_josiane.md) — drajosianetavares.com.br **NO AR**; repo em site-josiane-push (não mais Josiane Tavares/site-josiane); só SUS, sem agendamento e sem WhatsApp
...
- [Financeiro (app próprio)](financeiro_app.md) — Financeiro saiu do CRM e virou Web App em financeiro.tiagotavares.adv.br (mesmo repo/banco); ⚠️ LER `FINANCEIRO.md` no repo antes de mexer · cartão quita por FATURA, não por parcela
- [Senha admin tiagotavares](crm_admin_password.md) — `Ti882590@` p/ admin, CRM e financeiro; ⚠️ `$` e `#` são corrompidos pelo Hostinger e fazem a senha certa ser recusada
...
- [CRM - DOIS Sistemas + Migração Node](crm_dois_sistemas.md) — ⚠️ leitura pra retomar o CRM Node em produção. **✅ ROADMAP A→F COMPLETO (2026-07-04)**: Postgres/Supabase (projeto `crm-tiago`) · funil+cadastro+docs+tags · propostas+assinatura (PDF/pdfkit+e-mail/nodemailer) · painel Início/Tarefas/Anotações · Contatos · Processos+DataJud/CNJ · Google Agenda+Contatos (OAuth) · sessão persistente+senha via env. Tudo deployado e verificado em produção. Senha atual em [[crm_admin_password]] (não é mais `tiago2026`). **O Financeiro NÃO fica mais aqui** — virou app próprio, ver [[financeiro_app]]
- [Contas — Import de Extrato Bancário](project_contas_extrato_bancario.md) — app Contas pessoais (contas.tiagotavares.adv.br); parser do BB reescrito pro formato real do PDF, gravação implementada, bug do clique nos cards do painel corrigido; ⚠️ **NUNCA confirmado funcionando de ponta a ponta em produção** — próximo passo é testar o fluxo completo (importar → conferir → "Lançar os selecionados")
