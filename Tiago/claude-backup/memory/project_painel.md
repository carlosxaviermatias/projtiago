...
**🚨 INCIDENTE ABERTO (2026-06-07):** o serviço de Apps/Docker do TrueNAS caiu e o painel ficou fora do ar (NÃO é o nosso código — deploy já feito). Causa: `/run` (tmpfs) lotava em RAM baixa (3.7 GB) + race na detecção da interface de rede (DHCP) no TrueNAS 26 BETA. JÁ FEITO: liberei `/run`, dei sudo (no-password, REVERTER depois) ao `tiagotavares`, e troquei a interface enp2s0 de DHCP p/ **IP fixo 192.168.18.22/24** (gw/DNS 192.168.18.1) — isso deve matar a race. FALTA: re-disparar `sudo midclt call docker.update '{"pool":"PRINCIPAL"}'` e confirmar `docker.status`=RUNNING + painel no ar. **Detalhes completos e passo-a-passo de retomada em `_handoff/ESTADO-ATUAL.md` (seção "INCIDENTE ATIVO") — LER PRIMEIRO.**
